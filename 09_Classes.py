...
##### tested on 01/17/2024 -  Microsoft Windows - Python 3.12.0 (Oct  2 2023) #####
...
w2 = Warehouse()
w2.region = 'east'
print(w2.purpose, w2.region)  # w2.purpose will be like w1.purpose
...
f1(0, 8, 5)  # returns 8 whic is < 8 + 5
C.f(0, 3, 9) # returns 3 whic is < 3 + 9
C.g(0)   # returns 'hello world'
C.h(0)   # same as above
...
temp.addtwice('tissues')

temp.data  # is the list we created. temp.data is like self.data in the Class
...
# the global scope: for one thing, functions and modules imported into the global 
...
# a method of a base class that calls another method defined in the same 
...
# Base1, then (recursively) in the base classes of Base1, and if it was not found 
...
# accessed through multiple paths from the bottommost class). For example, all 
# classes inherit from object, so any case of multiple inheritance provides more 
# than one path to reach object. To keep the base classes from being accessed more 
...
# Python code: a name prefixed with an underscore (e.g. _spam) should be treated as 
# a non-public part of the API (whether it is a function, a method or a data member). 
# It should be considered an implementation detail and subject to change without 
# notice.
...
# a __update identifier since it is replaced with _Mapping__update in the 
...
# of the invoking class to be the current class; this is similar to the effect 
# of the global statement, the effect of which is likewise restricted to code 
...
# C “struct”, bundling together a few named data items. The idiomatic approach 
...
# if you have a function that formats some data from a file object, you can define 
# a class with methods read() and readline() that get the data from a string 
# buffer instead, and pass it as an argument.

# Instance method objects have attributes, too: m.__self__ is the instance object 
# with the method m(), and m.__func__ is the function object corresponding to the 
# method.
...
for key in {'one':1, 'two':2}:
    print(key)   # it prints one, two
  
for char in "123":
    print(char)  # it prints every single character: 1, 2, 3
  
for line in open("textfile.txt"):
    print(line, end='')   # it prints the text in the textfile.txt

# This style of access is clear, concise, and convenient. The use of iterators 
# pervades and unifies Python. Behind the scenes, the for statement calls iter() 
# on the container object. The function returns an iterator object that defines the 
...
# tells the for loop to terminate. You can call the __next__() method using the 
# next() built-in function; this example shows how it all works:

s = 'abc'
it = iter(s)
it   # result:  <str_ascii_iterator object at 0x000002272BC14E20>
...
class Reverse:
    ...
    def __init__(self, data):  # from example below: self = rev, data = 'spam'
        self.data = data  # self.data = 'spam'
        self.index = len(data)  # self.index = 4
    def __iter__(self):
        return self
    def __next__(self):
        if self.index == 0:  # 4 != 0, 3 != 0, 2 != 0, 1 != 0, 0 = 0 
            raise StopIteration  # self.index = 0 --> StopIteration
        self.index = self.index - 1  # self.index = 3, 2, 1, 
        return self.data[self.index]  # return self.data[3] which is 'm', 
                                      # return self.data[2] which is 'a',
                                      # return self.data[1] which is 'p',
                                      # return self.data[0] which is 's'.

rev = Reverse('spam')  # I pass 'spam' as argument in the Class 'spam' = data
...
# make it easy to create iterators with no more effort than writing a regular 
# function.
...
unique_words = set(word for line in page  for word in line.split())  # error

valedictorian = max((student.gpa, student.name) for student in graduates) # error

data = 'golf'
list(data[i] for i in range(len(data)-1, -1, -1))   # result is 'f' 'l' 'o' 'g'
