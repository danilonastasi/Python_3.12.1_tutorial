...
# writing (an existing file with the same name will be erased), and 'a' opens 
# the file for appending; any data written to the file is automatically added 
...
# not specified, the default is platform dependent (see open()). Because UTF-8 is 
# the modern de-facto standard, encoding="utf-8" is recommended unless you know 
# that you need to use a different encoding. Appending a 'b' to the mode opens 
# the file in binary mode. Binary mode data is read and written as bytes objects. 
...
# will corrupt binary data like that in JPEG or EXE files. Be very careful to 
...
# If you’re not using the with keyword, then you should call f.close() to close the 
# file and immediately free up any system resources used by it.

## Warning Calling f.write() without using the with keyword or calling f.close() 
# might result in the arguments of f.write() not being completely written to the 
...
# and returns it as a string (in text mode) or bytes object (in binary mode). size 
# is an optional numeric argument. When size is omitted or negative, the entire 
# contents of the file will be read and returned; it’s your problem if the file 
# is twice as large as your machine’s memory. Otherwise, at most size characters 
# (in text mode) or size bytes (in binary mode) are read and returned. If the end 
...
# at the end of the string, and is only omitted on the last line of the file if 
# the file doesn’t end in a newline. This makes the return value unambiguous; 
...
f.write('This is a test\n')  # it writes to the object f which is workfile
# 15
...
f.write(s)   # it writes to the object f which is workfile
# 18

f.tell()  # by the moment we run this code we have the file object f, 
          # workfile updated
...
# by the whence argument. A whence value of 0 measures from the beginning of the 
# file, 1 uses the current file position, and 2 uses the end of the file as the 
# reference point. whence can be omitted and defaults to 0, using the beginning 
# of the file as the reference point.

f = open('workfile', 'rb+')   # r=reading, b=binary mode

f.write(b'0123456789abcdef')  # b = binary

f.seek(5)      # Go to the 6th byte in the file

f.read(1)   # starts reading from the position we got in seek(5) above

f.seek(-3, 2)  # Go to the 3rd byte before the end

f.read(1)

# In text files (those opened without a b in the mode string), only seeks relative 
# to the beginning of the file are allowed (the exception being seeking to the very 
# file end with seek(0, 2)) and the only valid offset values are those returned from 
...
# are less frequently used; consult the Library Reference for a complete guide to 
# file objects.
...
# nested lists and dictionaries, parsing and serializing by hand becomes complicated.
...
# Python data hierarchies, and convert them to string representations; this process 
# is called serializing. Reconstructing the data from the string representation is 
# called deserializing. Between serializing and deserializing, the string 
# representing the object may have been stored in a file or data, or sent over 
# a network connection to some distant machine.

## Note: The JSON format is commonly used by modern applications to allow for 
# data exchange. Many programmers are already familiar with it, which makes it 
...
x = [1, 'simple', 'list']   
type(x)   # x is a list
json.dumps(x)  # convert all values in the list in a single string
type(json.dumps(x))   # is a string

# Another variant of the dumps() function, called dump(), simply serializes the 
# object to a text file. So if f is a text file object opened for writing, we can 
# do this:

# create a file txt with notepad in the path you are using
# after creating do:

f = open('workfile', 'w', encoding="utf-8") # creates a new file, empty
json.dump(x, f)  # inserts the string in the file created
f = open('workfile', 'r', encoding="utf-8")  # is saves and we call the 
                                             # file for reading
list(f)  we get the string ['[1, "simple", "list"]']
 
# To decode the object again, if f is a binary file or text file object which has 
# been opened for reading:

# f = open('workfile', 'r', encoding="utf-8")  
# or
f.seek(0)  # in order to move the pointer again at the beginning
x = json.load(f)   # we create a list from a string
type(x)  # now is again a list
print(x)

## Note: JSON files must be encoded in UTF-8. Use encoding="utf-8" when opening 
...
# arbitrarily complex Python objects. As such, it is specific to Python and cannot 
# be used to communicate with applications written in other languages. It is also 
# insecure by default: deserializing pickle data coming from an untrusted source 
# can execute arbitrary code, if the data was crafted by a skilled attacker.
