...
##### tested on 01/18/2024 -  Microsoft Windows - Python 3.12.0 (Oct  2 2023) #####
...
##### 12.3. Managing Packages with pip - continue #####
...
# If you re-run this command, pip will notice that the requested version is 
# already installed and do nothing. You can supply a different version number 
# to get that version, or you can run python -m pip install --upgrade to upgrade 
...
# followed by one or more package names will remove the packages from the 
# virtual environment.
...
python -m pip freeze 
# will produce a similar list of the installed packages, but the output uses the 
# format that python -m pip install expects. A common convention is to put this 
# list in a requirements.txt file:

# (tutorial-env) $ 
python -m pip freeze > requirements.txt
#(tutorial-env) $ 
cat requirements.txt  # error because cat is a Unix command and not Windows
# novas==3.1.1.3
# numpy==1.9.2
# requests==2.7.0

# The requirements.txt can then be committed to version control and shipped as 
# part of an application. Users can then install all the necessary packages with 
# install -r:
